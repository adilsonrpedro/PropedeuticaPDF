...
interface OcrTabProps { t?: any; onReviewSubmitted?: () => void; }
...
export default function OcrTab({ t, onReviewSubmitted }: OcrTabProps) {
  ...
  const safeStr = (key: string, fallback: string): string => {
    if (t && typeof t === 'object' && key in t) return t[key] || fallback;
    return fallback;
  };

  const handleCopy = async () => {
    await navigator.clipboard.writeText(ocrText); setCopied(true);
  ...
  const handleOcr = async () => {
    if (!file) return; setBusy('ocr'); setProgress(0); setError(''); setOcrText(''); setShowReview(false);
    try {
      const Tesseract = await import('tesseract.js');
      const worker = await Tesseract.createWorker(lang, 1, {
        logger: (m: { status: string; progress: number }) => {
          if (m.status === 'recognizing text') { setProgress(Math.round(m.progress * 100)); setStatusLabel(safeStr('ocrRecognizing', 'Reconhecendo texto...')); }
        },
      });
      ...
      if (ext === 'pdf') {
        ...
        setOcrText(data.text); setProgress(100);
      }
      await worker.terminate(); setShowReview(true);
    } catch (e) { setError(e instanceof Error ? e.message : safeStr('convError', 'Erro na conversão.')); } finally { setBusy('idle'); }
  };

  const handleSearchablePdf = async () => {
    if (!file || !file.name.toLowerCase().endsWith('.pdf')) return; setBusy('searchable'); setProgress(0); setError(''); setSearchablePdf(null); setShowReview(false);
    try {
      const Tesseract = await import('tesseract.js');
      const worker = await Tesseract.createWorker(lang, 1, { logger: (m: { status: string; progress: number }) => { if (m.status === 'recognizing text') setProgress(Math.round(m.progress * 100)); } });
      const pdfjs = await loadPdfjs(); const pdf = await pdfjs.getDocument({ data: await file.arrayBuffer() }).promise;
      const pdfDoc = await PDFDocument.create(); const font = await pdfDoc.embedFont(StandardFonts.Helvetica);
      for (let i = 1; i <= pdf.numPages; i++) {
        const canvas = await renderPage(pdf, i); const jpegBlob: Blob = await new Promise((resolve) => canvas.toBlob((b) => resolve(b!), 'image/jpeg', 0.85));
        const jpegBuf = await jpegBlob.arrayBuffer(); setStatusLabel(`${safeStr('ocrRecognizing', 'Reconhecendo texto...')} ${i}/${pdf.numPages}`);
        const { data } = await worker.recognize(jpegBlob as unknown as import('tesseract.js').ImageLike);
        const img = await pdfDoc.embedJpg(jpegBuf); const newPage = pdfDoc.addPage([img.width, img.height]);
        newPage.drawImage(img, { x: 0, y: 0, width: img.width, height: img.height });
        let y = img.height - 20;
        for (const line of data.text.split('\n')) { if (line.trim() && y > 0) { try { newPage.drawText(line.slice(0, 200), { x: 10, y, size: 4, font, color: rgb(1, 1, 1), opacity: 0.01 }); } catch { } y -= 6; } }
        setProgress(Math.round((i / pdf.numPages) * 100));
      }
      await worker.terminate(); await applyWatermarkToPdfDoc(pdfDoc);
      setSearchablePdf(await pdfDoc.save() as unknown as ArrayBuffer); setShowReview(true);
    } catch (e) { setError(e instanceof Error ? e.message : safeStr('convError', 'Erro na conversão.')); } finally { setBusy('idle'); }
  };

  const downloadSearchablePdf = () => {
    if (searchablePdf) { const baseName = file?.name.replace(/\.[^/.]+$/, '') || 'searchable'; saveAs(new Blob([searchablePdf], { type: 'application/pdf' }), `${baseName}_searchable.pdf`); }
  };

  const ocrLangs = [{ code: 'por', label: 'Português' }, { code: 'spa', label: 'Español' }, { code: 'eng', label: 'English' }];

  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6">
      <div className="mb-6 text-center">
        <h2 className="text-2xl font-bold text-slate-900 sm:text-3xl">{safeStr('ocrTitle', 'Reconhecimento de Texto (OCR)')}</h2>
        <p className="mt-2 text-sm text-slate-500">{safeStr('ocrSubtitle', 'Extraia texto de imagens ou transforme PDFs digitalizados.')}</p>
      </div>
      <DropZone onFile={setFile} title={safeStr('ocrDropTitle', 'Escolha uma imagem ou arquivo PDF')} subtitle={safeStr('ocrDropSubtitle', 'Arraste ou clique para carregar')} accept=".jpeg,.jpg,.png,.pdf" hasItems={!!file} accent="teal" />
      {file && (
        <div className="mt-5 space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="text-sm font-medium text-slate-700">Arquivo carregado: <span className="font-mono text-xs text-teal-600">{file.name}</span></div>
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-700">{safeStr('ocrLanguage', 'Idioma do Documento')}</label>
            <div className="flex flex-wrap gap-2">
              {ocrLangs.map((l) => (
                <button key={l.code} onClick={() => setLang(l.code)} className={`rounded-lg border px-3 py-2 text-sm font-medium transition ${lang === l.code ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>{l.label}</button>
              ))}
            </div>
          </div>
      ...
      {busy !== 'idle' && (
        <div className="mt-6 rounded-2xl border border-teal-100 bg-teal-50 p-4 text-center">
          ...
          <div className="w-full bg-teal-200 h-2 rounded-full overflow-hidden"><div className="bg-teal-600 h-2 transition-all duration-300" style={{ width: `${progress}%` }}></div></div>
          <div className="text-xs text-teal-600 mt-1">{progress}%</div>
        </div>
      )}
      {error && (
        <div className="mt-6 flex items-start gap-2 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
