...
export const ConvertSection: React.FC<ConvertSectionProps> = ({ t }) => {
  ...
  // Execução real da biblioteca pdf-lib (União dos PDFs)
  ...
  // Temporizador de 3 segundos no passo 'loading'
  useEffect(() => {
  ...
  // Download do arquivo unificado (Passo 3)
  const handleDownload = () => {
    if (!mergedBytes) return;

    const cleanFileName = outputFileName.trim() || 'PropedeuticaPDF-Unir';
    const finalName = cleanFileName.toLowerCase().endsWith('.pdf')
      ? cleanFileName
      : `${cleanFileName}.pdf`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 flex flex-col justify-between font-sans">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-8">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-200 ${
                isDragging
                  ...
                  : 'border-slate-300 dark:border-slate-700 hover:border-teal-400 dark:hover:border-teal-500 bg-white dark:bg-slate-800'
              }`}
            >
              <div className="flex flex-col items-center justify-center space-y-3">
                ...
                <div className="text-slate-700 dark:text-slate-200">
                  ...
                  ou arraste seus PDFs aqui
                </div>
                <p className="text-xs text-slate-400 dark:text-slate-500">
                  Selecione dois ou mais arquivos para juntar
                </p>
              </div>
            </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <ul className="divide-y divide-slate-100 dark:divide-slate-700 max-h-80 overflow-y-auto">
                  {files.map((file, index) => (
                    <li
                      key={`${file.name}-${index}`}
                      className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors"
                    >
                      ...
                      <div className="flex items-center space-x-1 flex-shrink-0">
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 disabled:opacity-30 transition-colors"
                        >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-end gap-4 pt-4">
              {files.length > 0 && (
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full sm:w-auto px-6 py-3 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 font-medium transition-colors flex items-center justify-center gap-2"
                >
          ...
          <div className="py-20 flex flex-col items-center justify-center text-center space-y-6 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                Processando e preparando seus documentos...
              </h2>
        ...
        {funnelStep === 'download' && (
          <div className="py-12 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm text-center space-y-6 max-w-2xl mx-auto">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                Seu PDF Unificado está Pronto!
              </h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Escolha o nome do seu arquivo e faça o download gratuito.
              </p>
            </div>
          ...
          <div className="py-10 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm text-center space-y-8 max-w-2xl mx-auto">
            <div className="space-y-3">
              ...
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">
                Obrigado por usar o PropedeuticaPDF!
              </h2>
              <p className="text-sm text-slate-600 dark:text-slate-300 max-w-md mx-auto">
                Seu download foi iniciado. Como foi sua experiência ao usar nossa ferramenta?
              </p>
            </div>

            {/* Formulário de Avaliação Supabase */}
            {!ratingSubmitted ? (
              <form onSubmit={handleSubmitRating} className="bg-slate-50 dark:bg-slate-900/60 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4 text-left">
                ...
                >
                  {isSubmittingRating ? (
                    ...
                    <span>Enviar Avaliação Anônima</span>
                  )}
                </button>
              </form>
            ) : (
            ...
            <div className="pt-2">
              <a
                href="/"
                className="inline-flex items-center justify-center gap-2 px-6 py-3 border border-slate-300 dark:border-slate-600 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
              >
      ...
      {/* Rodapé Fixo com Instagram Oficial */}
      <footer className="border-t border-slate-200 dark:border-slate-800 py-6 bg-white dark:bg-slate-900 text-center text-xs text-slate-500 dark:text-slate-400 space-y-2">
